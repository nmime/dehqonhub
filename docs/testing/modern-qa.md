...
This repository treats QA as runner-neutral and contains no GitHub Actions. Every preset below is a real command that runs from a developer workstation or a separately configured trusted runner with the required runtime.
...
| Cross-browser/mobile e2e   | Chromium, 320px Chromium, Firefox, WebKit, mobile Chrome, mobile Safari             | `pnpm run test:e2e:matrix`        | Manual/nightly                    | Uses `playwright.extended.config.ts`; set `PLAYWRIGHT_BASE_URL` to derive the selected user/admin app and user/admin/auth API URLs for an existing stack, provide that complete explicit URL set, or set `PLAYWRIGHT_MANAGE_STACK=1` for selected-closure Docker management. Landing and site renderer suites stay project-local and are not external-matrix prerequisites. Set `PLAYWRIGHT_INCLUDE_QUARANTINED=1` to include `@quarantine` specs. Use `-- --dry-run` to print the command. |
