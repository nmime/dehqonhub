...
describe('MarketplaceService', () => {
  ...
  it('covers the complete read, mutation, validation, and repository-result boundary', async () => {
    const { repository, service } = fixture();
    repository.roleOf.mockResolvedValue('buyer');
    repository.isApprovedOrganization.mockResolvedValue(true);
    repository.listVerifications.mockResolvedValue([]);
    repository.listCarts.mockResolvedValue([]);
    repository.listRequests.mockResolvedValue([]);
    repository.listMyRequests.mockResolvedValue([]);
    repository.listContracts.mockResolvedValue([]);
    repository.listTenantContracts.mockResolvedValue([]);
    repository.getCart.mockResolvedValue({ id: 'cart-1' });
    repository.updateCartItem.mockResolvedValue(ok({ id: 'cart-1' }));
    repository.removeCartItem.mockResolvedValue(ok({ id: 'cart-1' }));
    repository.listOffers.mockResolvedValue(ok([]));

    await expect(service.listVerifications(owner.tenantId)).resolves.toEqual([]);
    await expect(service.requireVerified(owner)).resolves.toBeUndefined();
    await expect(service.listCarts(owner)).resolves.toEqual([]);
    await expect(service.getCart(owner, 'cart-1')).resolves.toMatchObject({ id: 'cart-1' });
    await expect(service.updateCartItem(owner, 'cart-1', 'listing-1', 2, 'cart-update-1001')).resolves.toMatchObject({
      id: 'cart-1',
    });
    await expect(service.removeCartItem(owner, 'cart-1', 'listing-1', 'cart-remove-1001')).resolves.toMatchObject({
      id: 'cart-1',
    });
    await expect(service.listRequests(owner.tenantId, 'all')).resolves.toEqual([]);
    await expect(service.listMyRequests(owner)).resolves.toEqual([]);
    await expect(service.listOffers(owner, 'request-1')).resolves.toEqual([]);
    await expect(service.listContracts(owner)).resolves.toEqual([]);
    await expect(service.listTenantContracts(owner.tenantId)).resolves.toEqual([]);

    await expect(
      (
        service as unknown as {
          requireBuyerRole: (candidate: typeof owner) => Promise<string>;
        }
      ).requireBuyerRole(owner),
    ).resolves.toBe('buyer');

    repository.roleOf.mockResolvedValueOnce(undefined);
    await expect(service.requireVerified(owner)).rejects.toBeInstanceOf(ForbiddenException);
    repository.getCart.mockResolvedValueOnce(undefined);
    await expect(service.getCart(owner, 'missing')).rejects.toBeInstanceOf(ResourceNotFoundException);
    for (const expectedRevision of [-1, 0.5]) {
      await expect(
        service.reviewVerification(
          ...
          'reviewer-1',
          expectedRevision,
          `verification-review-${String(expectedRevision)}-key`,
        ),
    ...
    await expect(
      service.addToCart(
        owner,
        { actingPartnerId: 'buyer-partner', listingPublicationId: 'listing-1', quantity: 0 },
        'cart-add-1001',
      ),
    ).rejects.toBeInstanceOf(BadRequestException);
    ...
    for (const expectedRevision of [-1, 0.5]) {
      await expect(
        service.updateContractDeliveryQuote(
          owner,
          'contract-1',
          { deliveryPriceUzs: 1, expectedRevision },
          `quote-${String(expectedRevision).replace('.', '-')}-key`,
        ),
      ).rejects.toBeInstanceOf(BadRequestException);
    }

    repository.roleOf.mockResolvedValue('buyer');
    repository.isApprovedOrganization.mockResolvedValueOnce(false);
    await expect(
      (
        service as unknown as {
          requireBuyerRole: (candidate: typeof owner) => Promise<string>;
    ...
    const unwrapCases = [
      ...
      [{ status: 'invalid_state', field: 'quantity' }, BadRequestException],
      [{ status: 'partner_unapproved' }, BadRequestException],
    ] as const;
    for (const [result, ErrorType] of unwrapCases) {
      repository.updateCartItem.mockResolvedValueOnce(result);
      await expect(
        service.updateCartItem(owner, 'cart-1', 'listing-1', 2, `cart-result-${result.status}-key`),
      ).rejects.toBeInstanceOf(ErrorType);
    }

    repository.roleOf.mockResolvedValueOnce(undefined);
    await expect(service.makeOffer(owner, 'request-1', {} as never, 'offer-role-1001')).rejects.toBeInstanceOf(
      ForbiddenException,
    );
  });
});
