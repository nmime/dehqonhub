...
function partnerEntity(overrides: Record<string, unknown> = {}) {
  return {
    id: 'partner-1', tenantId: 'tenant-1', ownerUserId: 'user-1', kind: 'supplier',
    legalName: 'Agro Supply', taxId: '123456789', phone: '+998901234567', region: 'Fergana',
    status: 'approved', reviewedBy: null, reviewedAt: null, createdAt: now, updatedAt: now,
    ...overrides,
  };
}
...
function farmerEntity(overrides: Record<string, unknown> = {}) {
  return {
    id: 'farmer-1', tenantId: 'tenant-1', userId: 'user-1', firstName: 'Ali', lastName: 'Valiyev',
    phone: '+998901234567', region: 'Fergana', district: null, crops: ['cotton'], status: 'active',
    fieldAgentUserId: null, createdAt: now, updatedAt: now,
    ...overrides,
  };
}
...
describe('PostgresAgriTechOperationsRepository — partners', () => {
  ...
  it('creates a partner when taxId is unique within tenant+kind', async () => {
    ...
    expect(em.findOne).toHaveBeenCalledWith(expect.anything(), { tenantId: 'tenant-1', kind: 'supplier', taxId: '999' });
...
describe('PostgresAgriTechOperationsRepository — supplier products', () => {
  ...
  const productInput = {
    partnerId: 'partner-1', name: 'NitroAmmonka', category: 'fertilizer' as const,
    description: 'N46', priceUzs: 85000, unit: '50kg bag', stockQuantity: 100, region: 'Fergana',
  };
  ...
  it('lists supplier products only for owned partners', async () => {
    em.find
      .mockResolvedValueOnce([partnerEntity()])
  ...
  it('updates price and stock of an owned product', async () => {
    const product = { id: 'prod-1', tenantId: 'tenant-1', supplierId: 'partner-1', name: 'Seed', nameRu: null, nameUz: null, category: 'seed', description: 'd', priceUzs: 1000, unit: 'kg', stockQuantity: 5, region: 'R', status: 'active' };
    em.findOne
      .mockResolvedValueOnce(product)
      .mockResolvedValueOnce(partnerEntity({ status: 'approved' }));
    const result = await repo.updateSupplierProduct(owner, 'prod-1', { priceUzs: 2000, stockQuantity: 10, status: 'active' });
    expect(result.status).toBe('ok');
  ...
  it('returns not_found when updating a missing product', async () => {
    em.findOne.mockResolvedValue(null);
    const result = await repo.updateSupplierProduct(owner, 'missing', { priceUzs: 1, stockQuantity: 1, status: 'active' });
    expect(result).toEqual({ status: 'not_found' });
  });
  ...
  it('returns forbidden when product belongs to another owner', async () => {
    ...
    const result = await repo.updateSupplierProduct(owner, 'prod-1', { priceUzs: 1, stockQuantity: 1, status: 'active' });
...
describe('PostgresAgriTechOperationsRepository — produce listings', () => {
  ...
  const listingInput = {
    crop: 'cotton', grade: 'A' as const, quantityKg: 100, pricePerKgUzs: 5000,
    region: 'Fergana', availableFrom: now, availableUntil: later,
  };
  ...
  it('discovers median price across active listings', async () => {
    ...
    em.find.mockResolvedValue([
      { id: 'l1', tenantId: 'tenant-1', farmerId: 'f1', crop: 'cotton', grade: 'A', quantityKg: 100, availableQuantityKg: 50, pricePerKgUzs: 4000, region: 'Fergana', availableFrom: now, availableUntil: later, status: 'active', createdAt: now, updatedAt: now },
      { id: 'l2', tenantId: 'tenant-1', farmerId: 'f2', crop: 'cotton', grade: 'A', quantityKg: 100, availableQuantityKg: 30, pricePerKgUzs: 6000, region: 'Fergana', availableFrom: now, availableUntil: later, status: 'active', createdAt: now, updatedAt: now },
      { id: 'l3', tenantId: 'tenant-1', farmerId: 'f3', crop: 'cotton', grade: 'A', quantityKg: 100, availableQuantityKg: 10, pricePerKgUzs: 5000, region: 'Fergana', availableFrom: now, availableUntil: later, status: 'active', createdAt: now, updatedAt: now },
    ]);
  ...
  it('cancels an untouched listing', async () => {
    const listing = { id: 'l1', tenantId: 'tenant-1', farmerId: 'farmer-1', crop: 'cotton', grade: 'A', quantityKg: 100, availableQuantityKg: 100, pricePerKgUzs: 5000, region: 'R', availableFrom: now, availableUntil: later, status: 'active', createdAt: now, updatedAt: now };
    em.findOne
...
describe('PostgresAgriTechOperationsRepository — produce reservation', () => {
  ...
  function listing(overrides: Record<string, unknown> = {}) {
    return {
      id: 'l1', tenantId: 'tenant-1', farmerId: 'farmer-1', crop: 'cotton', grade: 'A',
      quantityKg: 100, availableQuantityKg: 100, pricePerKgUzs: 5000, region: 'Fergana',
      availableFrom: now, availableUntil: later, status: 'active', createdAt: now, updatedAt: now,
      ...overrides,
    };
  }
  ...
  it('reserves quantity atomically and creates a confirmed order', async () => {
    ...
    em.findOne
      ...
      .mockResolvedValueOnce(listing());
    const result = await repo.reserveProduce(owner, 'l1', { partnerId: 'partner-1', quantityKg: 25, deliveryAddress: 'Toshkent' });
  ...
  it('rejects reservation when buyer partner missing', async () => {
    em.findOne.mockResolvedValue(null);
    const result = await repo.reserveProduce(owner, 'l1', { partnerId: 'missing', quantityKg: 10, deliveryAddress: 'T' });
    expect(result).toEqual({ status: 'not_found' });
  });

  it('rejects reservation from unapproved buyer', async () => {
    em.findOne.mockResolvedValue(partnerEntity({ kind: 'buyer', status: 'pending' }));
    const result = await repo.reserveProduce(owner, 'l1', { partnerId: 'partner-1', quantityKg: 10, deliveryAddress: 'T' });
    expect(result).toEqual({ status: 'partner_unapproved' });
  });
  ...
  it('rejects overselling with insufficient_quantity', async () => {
    ...
    const result = await repo.reserveProduce(owner, 'l1', { partnerId: 'partner-1', quantityKg: 25, deliveryAddress: 'T' });
...
describe('PostgresAgriTechOperationsRepository — farmers and field visits', () => {
  ...
  it('lists farmers assigned to the field agent', async () => {
    ...
    expect(em.find).toHaveBeenCalledWith(expect.anything(), { tenantId: 'tenant-1', fieldAgentUserId: 'user-1' }, expect.anything());
    expect(result).toHaveLength(1);
  });
  ...
  it('records a field visit only for the assigned agent', async () => {
    ...
    const result = await repo.recordFieldVisit(owner, { farmerId: 'farmer-1', notes: 'Healthy cotton', observedGrade: 'A', observedAt: now });
    expect(result.status).toBe('ok');
    expect(em.persist).toHaveBeenCalled();
  });
...
describe('PostgresAgriTechOperationsRepository — deliveries', () => {
  ...
  it('transitions delivery with proof and appends history', async () => {
    const delivery = { id: 'd1', tenantId: 'tenant-1', orderId: 'order-1', agentUserId: 'user-1', status: 'in_transit', scheduledAt: now, proofReference: null, history: [], createdAt: now, updatedAt: now };
    em.findOne
      .mockResolvedValueOnce(delivery)
      .mockResolvedValueOnce({ ...order, history: [] });
...
describe('PostgresAgriTechOperationsRepository — advisories', () => {
  ...
  const advisoryInput = { farmerId: 'farmer-1', kind: 'weather' as const, source: 'hydromet', summary: 'Rain', observedAt: now, expiresAt: later };
  ...
  it('lists advisories for the farmer identity', async () => {
    em.findOne.mockResolvedValue(farmerEntity());
    em.find.mockResolvedValue([{ id: 'a1', tenantId: 'tenant-1', farmerId: 'farmer-1', kind: 'weather', source: 'hydromet', summary: 'Rain', observedAt: now, expiresAt: later, createdAt: now }]);
...
describe('PostgresAgriTechOperationsRepository — analytics', () => {
  it('aggregates tenant KPIs with GMV and commission', async () => {
    const em = makeEm();
    em.find
      .mockResolvedValueOnce([farmerEntity({ status: 'active' }), farmerEntity({ id: 'f2', status: 'pending_verification' })])
      .mockResolvedValueOnce([partnerEntity({ kind: 'supplier', status: 'approved' }), partnerEntity({ id: 'p2', kind: 'buyer', status: 'pending' })])
      .mockResolvedValueOnce([{ id: 'prod1', stockQuantity: 50 }, { id: 'prod2', stockQuantity: 30 }])
...
describe('PostgresAgriTechOperationsRepository — pilots and integrations', () => {
  ...
  it('transitions pilot status when allowed', async () => {
    const pilot = { id: 'p1', tenantId: 'tenant-1', name: 'pilot', status: 'planned', targetFarmers: 10, targetSuppliers: 2, startsAt: now, endsAt: later, createdAt: now, updatedAt: now };
