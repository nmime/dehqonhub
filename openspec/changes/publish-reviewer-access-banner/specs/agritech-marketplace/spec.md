...
REQ-AGRITECH-MARKETPLACE-016 as visually distinct sub-carts, each naming its
seller with the verified seal the listing projection reports, its own line
items, its own item count, and its own total. Exactly one sub-cart SHALL be
active: it stays expanded with the delivery choice and the checkout action while
every other sub-cart collapses to a labelled count-and-total row with one swap
control. The active selection SHALL persist in versioned browser storage on the
same fail-closed discipline as the local preview cart, and SHALL resolve
deterministically to the first remaining cart when the stored one is gone.
...
- Guest preview carts are bounded and versioned, remain device-local, group
  ...
  button.
- The cart route never merges two sellers into one prospective order. The
  sub-cart switcher is a tablist with roving tab focus and Arrow/Home/End keys,
  the active sub-cart is announced in a polite live region, and every count and
  total is rendered inside the sub-cart it belongs to. A single seller drops the
  switcher yet still names that seller. A line whose listing left the
...
- **THEN** only the chosen sub-cart stays expanded with its own count and total,
  the others remain collapsed summary rows, the selection survives the reload
  from versioned browser storage, and checkout submits that one seller's cart id
  without ever mixing sellers into a single order
